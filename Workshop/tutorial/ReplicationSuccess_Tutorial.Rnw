...
\item Design prior
\begin{itemize}
\item[] $\rightarrow$ {\color{red} \emph{Conditional}}: ignores uncertainty of original study
\item[] $\rightarrow$ {\color{red} \emph{Predictive}}: reflects that there is 
...
\framesubtitle{(Solutions available at \url{https://osf.io/})}
\begin{block}{}
...
\end{block}
  ...
  \begin{block}{Exercise 1.2}
    How many study pairs show \emph{compatible effect estimates}? Use the function \texttt{predictionInterval()} with design prior set to \texttt{"predictive"}.
  \end{block}
\end{frame}
...
For your favourite replication project
  \begin{block}{Exercise 1.3}
    Compute the one-sided \emph{sceptical $p$-value}. How many replication studies are successful at the nominal and controlled threshold? 
...
\begin{frame}[fragile]{Solution: Discrepant studies}
<< echo = FALSE, fig.height = 4 >>=
for (p in unique(RProjects$project)) {
  data_project <- subset(RProjects, project == p)
  cat(paste0(p, ": \n"))
...
\begin{frame}{Statistical significance: Functions}
  \begin{block}{}
Two functions: 
    \begin{itemize}
...
plot(po.plot, pow.cond*100, col = "red", ylim = c(0,100), type = "l", 
    ...
    cex.lab = 1.5, 
     cex.axis = 1.5)
lines(po.plot, pow.condshrink*100, col = "red", lty = 3,
      lwd = 2)
abline(h = 50, lty = 2)
axis(2, at = 50, label = "50", cex.axis = 1.5,
     col = "gray40")
...
plot(po.plot, ss.cond, type = "l", ylim = c(0, 4), col = "red", 
     ...
     cex.lab = 1.5, 
     cex.axis = 1.5)
lines(po.plot, ss.condshrink, col = "red", lty = 3, 
      lwd = 2)
legend("topleft", 
        c("Conditional without shrinkage", "Conditional with 25% shrinkage"), 
        col = c("red", "red"), 
...
\begin{frame}{Replication Success: Functions}
\begin{block}{}
Two functions:
    \begin{itemize}
...
\begin{frame}{Exercises}
  ...
  For the experimental philosophy replication project
  \begin{itemize}
  \item Calculate for each study the required relative sample size to reach a power for replication success of 95\%. Use the conditional design prior the controlled threshold ($\alpha$ = 0.065). Use the function \texttt{sampleSizeReplicationSuccess()}
  \item Compare them to the actually used relative sample sizes.
  \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}[fragile]{Solution: Exercise 2.4}
\framesubtitle{Statistical significance}
