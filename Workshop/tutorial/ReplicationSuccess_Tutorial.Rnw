...
\begin{itemize}
  \item Functionalities for design and analysis of replication studies \\
...
# power calculation
powerSignificance(zo = p2z(0.009), c = 3, designPrior = "conditional")


# sample size calculation
sampleSizeSignificance(zo = p2z(0.009), power = 0.9, designPrior = "predictive")
@
...
\begin{frame}{2. Sceptical $p$-value}
\begin{block}{}
Two functions:
    \begin{itemize}
...
\begin{frame}[fragile]{2. Sceptical $p$-value}
Example from Morewedge et al. (2010)
    \begin{itemize}
      \item $p$-value $p_o =$ \Sexpr{round(study$po, 3)}
      \item relative sample size $c = $ \Sexpr{round(c,1)}
