...
@
\pause
\item $p$-value $p_o$ or $z$-value $z_o$ of original study
<< size = "small" >>=
RProjects$zo <- RProjects$fiso/RProjects$se_fiso
RProjects$po <- z2p(RProjects$zo)
@
\pause
\item $p$-value $p_r$ or $z$-value $z_r$ of replication study
<< size = "small" >>=
...
\begin{itemize}
\item Compute and plot the replication sample sizes of the six studies to achieve a power of 80\% with the conditional and the predictive design prior. 
\item What happens if we decide to shrink the original effect estimate to 75\% of the reported value? 
\end{itemize}
  \end{block}
