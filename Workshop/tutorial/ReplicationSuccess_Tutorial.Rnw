...
\begin{frame}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Part I}}
   \end{center}
  \end{block}
\end{frame}
...
$\rightarrow$ $\alpha = 0.05^2 = 0.0025$?
\end{block}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
<< fig.height = 4.5, echo = FALSE >>=
...
\framesubtitle{New definition of replication success}
  \centering
\includegraphics[width=0.9\textwidth]{images_presentation/Held2019.png}
\end{frame}
...
$\rightarrow$ returns one-sided sceptical $p$-value per default
\end{block}
\vspace{-.5cm}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
...
\begin{frame}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Application}}
   \end{center}
  \end{block}
\end{frame}
...
\begin{itemize}
  \item Statistical methods and functionality for design and analysis of replication studies \\
...
\item Design prior
\begin{itemize}
\item[] $\rightarrow$ \emph{Conditional}: ignores uncertainty of original study
\item[] $\rightarrow$ \emph{Predictive}: reflects that there is 
...
@
\pause
\item $z$-value $z_o$ or $p$-value $p_o$ of original study
<< size = "small" >>=
RProjects$zo <- RProjects$fiso/RProjects$se_fiso
RProjects$po <- z2p(RProjects$zo)
@
\pause
\item $z$-value $z_r$ or $p$-value $p_r$ of replication study
<< size = "small" >>=
RProjects$zr <- RProjects$fisr/RProjects$se_fisr
RProjects$pr <- z2p(RProjects$zr)
@
...
\begin{frame}[fragile]{Solution: Statistical significance}
<< echo = FALSE >>=
...
\begin{frame}[fragile]{Solution: Prediction intervals}
<< echo = FALSE >>=
## computing zo, zr, c
...
\begin{frame}[fragile]{Solution: Sceptical $p$-value}
...
\begin{frame}[fragile]{Solution: Discordant studies}
<< echo = FALSE, fig.height = 4 >>=
for (p in unique(RProjects$project)) {
  data_project <- subset(RProjects, project == p)
  cat(paste0(p, ": \n"))
...
\begin{frame}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Part II}}
   \end{center}
  \end{block}
\end{frame}
...
\begin{frame}{Design of replication studies}
\begin{block}{Issues with standard sample size calculation}
  \begin{itemize}
...
\begin{frame}{Application}
 \begin{enumerate}
    \item Statistical significance
    \item Sceptical $p$-value (replication success)
  \end{enumerate}
\end{frame}
...
Example from Morewedge et al. (2010)
    \begin{itemize}
      \item $p$-value $p_o =$ \Sexpr{round(study$po, 3)}
...
# power calculation
powerSignificance(zo = p2z(0.009), c = 3, 
                  designPrior = "conditional")


# sample size calculation
sampleSizeSignificance(zo = p2z(0.009), power = 0.9, 
...
\begin{itemize}
\item Compute the conditional and predictive power of the six replication studies and plot it as a function of the original $p$-value.
...
\begin{frame}[fragile]{1. Statistical significance}
  \begin{block}{Exercise 1.1 - \color{red}{Solutions}}
\begin{itemize}
\item Compute the conditional and predictive power of the six replication studies and plot it. What do you notice? % Goal: bordeline significant original study: very low power with both methods
...
\begin{frame}[fragile]
<<echo = FALSE>>=
po.reynolds <- reynolds$po
@

 \begin{block}{Exercise 1.5 - \color{red}{Solutions}}
  \begin{itemize}
  \item Compute the required replication sample size to reach a power of 95\% for this study with the conditional and the predictive design prior.
...
\begin{frame}{2. Replication Success}
\begin{block}{}
Two functions:
    \begin{itemize}
...
\begin{frame}[fragile]{2. Replication Success}
Example from Morewedge et al. (2010)
    \begin{itemize}
      \item $p$-value $p_o =$ \Sexpr{round(study$po, 3)}
      \item relative sample size $c = $ \Sexpr{round(c,1)}
...
<<>>=
# sample size calculation
sampleSizeReplicationSuccess(zo = p2z(0.009), power = 0.8,
                             designPrior = "predictive",
                             alternative = "one.sided", 
                             level = 0.065)
@
...
\begin{frame}[fragile]{2. Replication Success}
  \begin{block}{Exercise 2.1 - \color{red}{Solutions}}
    \begin{itemize}
    \item Compute and plot the conditional and predictive power for replication success of the 6 studies from exercise 1.1 as a function of the two-sided $p$-value using the nominal (0.025) and controlled (0.065) threshold. 
    \end{itemize}
<<echo = F, fig.height = 5>>=
...
\begin{frame}[fragile]{2. Replication Success}
  \begin{block}{Exercise 2.2 - \color{red}{Solutions}}
\vspace{.5cm}
<<echo = FALSE, fig.height=6>>=
...
\begin{frame}[fragile]{2. Replication Success}
  \begin{block}{Exercise 2.3 - \color{red}{Solutions}}
    \vspace{.5cm}
<<fig.height = 6, echo = F>>=
...
% ============================================================================
\nocite{Pawel2019, Held2020, Opensc2015, Camerer2016, Camerer2018, Cova2018, Morewedge2010, Reynolds2008}
