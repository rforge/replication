...
{\large \textit{Are the effect estimates statistically compatible?}} \\

% $\rightarrow$ function: \texttt{predictionInterval()} \\
% $\rightarrow$ function: \texttt{Qtest()}
Replication effect contained in prediction interval? \\ 
$\rightarrow$ function \texttt{predictionInterval()} \\
$p$-value from meta-analytic $Q$-test statistically significant? \\ 
$\rightarrow$ function \texttt{Qtest()}
\end{block}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
...
ggplot(data = plot_df, aes(x = Study, y = r)) +
  ...
  geom_text(data = pi_df, color = "springgreen4",
            ...
            nudge_y = 0.4, size = 14) +
  annotate(geom = "text", x = 1.5, y = 0.25, 
...
$\rightarrow$ recalibrated $p_S$ computed by default
 \vspace{.15cm}
\begin{center}
{\color{red} If $p_S \leq \alpha$ we have replication success at level $\alpha$} 
% Recalibrated $p_S$ are computed by default
\end{center}
\end{block}
\end{minipage}
\begin{minipage}[t][6cm][t]{\textwidth}
...
% Removed for tutorial in Frankfurt
% \begin{frame}{4. Sceptical $p$-value}
% \framesubtitle{New definition of replication success}
% 
% \begin{minipage}[t][2.7cm][t]{\textwidth}
...
\begin{block}{}
  \begin{itemize}
    ...
    \item $Q$-test: Can only quantify evidence \emph{for} incompatibility
    \item Meta-analysis: Exchangability assumption questionable
    \item Sceptical $p$-value penalizes effect size shrinkage
...
ggplot(data = plot_df, aes(x = Study, y = r)) +
  ...
  geom_text(data = ma_data, color = "tan3",
          ...
          nudge_y = -0.31, nudge_x = 0.12, 
          size = 10, parse = TRUE) +
    annotate(geom = "text", x = 1.5, y = 0.65, 
             label = paste("italic(p)[Q] ==~", formatPval(q_pval)),
...
ggplot(data = plot_df, aes(x = Study, y = z)) +
  ...
  geom_text(aes(label = paste("italic(z) ==~", 
                              round(z, 2), 
                              sep = "")), parse = TRUE, 
            nudge_x = c(-0.31, 0.27), nudge_y = -0.1, size = 10) +
...
\end{block}
  \begin{block}{Exercise 1.1}
     How many study pairs fulfill the \emph{significance} criterion for 
     replication success? Use a cut-off of $0.025$ for the one-sided 
     $p$-values.
  \end{block}
  \begin{block}{Exercise 1.2}
    How many study pairs show \emph{compatible effect estimates} (on Fisher 
    $z$-scale)? Use the function
    \texttt{Qtest()} and a cut-off of 0.05 for the resulting $p$-value.
  \end{block}
\end{frame}
...
## "technically" not a success but just a failure to reject the null
RProjects$Qsuccess <- RProjects$pQ >= 0.05
allQDF <- data.frame(project = "all", success = mean(RProjects$Qsuccess)*100, 
                     type = "Q-test",
                     successFormat = paste0(round(mean(RProjects$Qsuccess)*100, 0),
                                           "\\% (", sum(RProjects$Qsuccess),
                                           "/", length(RProjects$Qsuccess), ")"))

## compute for each project
QList <- lapply(X = unique(RProjects$project), FUN = function(p) {
  data_project <- subset(RProjects, project == p)
  data.frame(project = p, success = mean(data_project$Qsuccess)*100, type = "Q-test",
             successFormat = paste0(round(mean(data_project$Qsuccess)*100, 0), 
                                    "\\% (", sum(data_project$Qsuccess), "/", 
                                    length(data_project$Qsuccess), ")"))
})
...
colnames(Table2) <- c("Project", "Estimates ``compatible'' ($p_Q \\geq 0.05$)")
...
for (p in unique(RProjects$project)) {
  ...
  # abline(v = 0, lty = 3)
  title <- paste0(p, ": ", round(coverage*100, 0), '% "compatible"')
  plot(rr ~ ro, data = data_project, ylim = c(-0.5, 1), cex = 2.5,
       xlim = c(-0.5, 1), main = title, xlab = expression(italic(r)[o]),
       ylab = expression(italic(r)[r]), col = color, pch = 20,
       cex.main = 1.5, cex.axis = 1.5, cex.lab = 1.5)
  legend("topleft", #c("incompatible", "compatible"), 
...
For all studies from the replication projects investigate
  \begin{block}{Exercise 1.3}
    Compute the one-sided \emph{sceptical $p$-value}. How many replication 
    studies are successful at $0.025$? 
      Use the function \texttt{pSceptical()}
  \end{block}
  \begin{block}{Exercise 1.4}
    Look closer at the studies which show discrepancies in terms of replication 
...
\vspace{2cm}
  ...
  {\LARGE Design based on significance}
   ...
   \vspace{2cm}
   \begin{center}
{\color{red} \small \emph{Poll: how should we choose the sample size of 
...
\item Shrink the original effect estimate to 75\% of the reported value and use a conditonal design prior. How does the required relative sample size change compared to not shrinking the estimate? {\color{red} remove exercise for time reason if needed} 
\end{itemize}
  \end{block}
