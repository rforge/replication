...
\begin{block}{Replication sample size}
  \begin{itemize}
...
\begin{frame}{Approaches (title not optimal)}
\begin{block}{1. Statistical significance}
Two functions: 
\begin{itemize}
...
Main arguments
\begin{itemize}
\item \texttt{po} or \texttt{to}: $p$-value or test statistic of the original study
\item \texttt{c}: relative sample size $n_r/n_o$ (only for \texttt{powerSignificance})
\item \texttt{power}: desired level of replication power (only for \texttt{sampleSizeSignificance()})
\item \texttt{designPrior}: conditional or predictive
