...
$\rightarrow$ Fisher's $z$-transformation for correlation coefficients $r$ with (effective) sample size $n - 3$
\end{itemize}
\end{block}
<< fig.height = 4.5, echo = FALSE>>=
r <- seq(-1, 1, 0.001)
...
RProjects$ps <- with(RProjects, pSceptical(zo = zo, zr = zr, c = c, 
                                           alternative = "one.sided"))
thresh_nominal <- thresholdSceptical(level = 0.025, type="nominal", 
                                     alternative = "one.sided")
