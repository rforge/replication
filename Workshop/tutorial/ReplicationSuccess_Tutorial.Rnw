...
\section{Introduction}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Introduction}}   \footnote{Poll 1}  
   \end{center}
  \end{block}
\end{frame}
...
ggplot(data = plot_df, aes(x = Study, y = z)) +
  ...
  geom_text(aes(label = paste("italic(fis) ==~", 
                              round(z, 2), 
                              sep = "")), parse = TRUE, 
            nudge_x = c(-0.31, 0.27), nudge_y = -0.1, size = 10) +
...
\section{Exercise 2 -- Design of replication studies}
  % \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Exercise Session 2}} \footnote{  {Poll 4}}
  \\~\\
