...
\begin{frame}
  \begin{block}{}
  \begin{center}
  \color{uzh@blue}{\Huge \textbf{Background}}
   \end{center}
  \end{block}
\end{frame}
...
\begin{itemize}
  ...
  $\rightarrow$ Sceptical $p$-value \citep{Held2019}
  \end{itemize}
  \centering
...
@
\pause
\item $p$-value or test statistic of replication study
<<>>=
...
Example from Morewedge et al. (2010)
    \begin{itemize}
      \item $t_o =$ \Sexpr{round(study$z_O/study$z_se_O, 2)}
      \item $p_o =$ \Sexpr{round(study$pval_O, 3)}
...
\begin{frame}[fragile]{1. Statistical significance}
  \begin{block}{Exercise 1.1 - \color{red}{Solutions}}
\begin{itemize}
\item What happens if we decide to take less subjects in the replication study as compared to the original study?\\
\color{red}{c = 0.6} % Goal: if conditional power lower than 50% -> predictive power larger than conditional power. 
...
\end{itemize}
  \end{block}
<<echo = FALSE, fig.height = 5, eval = FALSE>>=
sampleSizeSignificance(po = eco$pval_O, power = 0.9, designPrior = "conditional")

sampleSizeSignificance(po = eco$pval_O, power = 0.9, designPrior = "predictive")

@


$\rightarrow$ Most of the required replication sample size are above one with conditional design prior \\
$\rightarrow$ Predictive design prior gives larger sample sizes than conditional design prior
\end{frame}
...
plot(required_c, eco$c, pch = 19, ylim = c(0, 4), xlim = c(0, 25),
     xlab = "required", ylab = "actual", cex = 1.2, cex.lab = 1.5, 
     cex.axis = 1.5)
...
for (p in unique(ReplicationProjects$project)) {
  ...
  study2 <- seq(1, nrow(data_project))
  plot(data_project$r_R, study2, col = color, pch = 20, 
       xlim = c(-0.5, 1), xlab = expression(italic(r)[r]), 
       main = paste0(p, ": ", round(coverage*100, 1), "% coverage"),
       cex.main = 1.5)
  arrows(PI$lower, study2, PI$upper, study2, length = 0.02, angle = 90, code = 3, col = color)
  points(data_project$r_R[paradox], study2[paradox],
         col = "purple3", cex = 1.5)
  arrows(x0 = data_project$r_R[paradox] - 0.1, 
         x1 = data_project$r_R[paradox] - 0.03,
         y0 = study2[paradox] + 2, y1 = study2[paradox] + 0.5,
...
\begin{itemize}
  ...
  $\rightarrow$ argument in most functions \texttt{d}
  % $= \tau^2/\sigma^2_o$
