...
\item Shrink the original effect estimate by a factor of 25\% and use a conditonal design prior. How does the power compare to the conditional power without shrinkage?  
\end{itemize}
  \end{block}
<<echo = FALSE, eval = F>>=
...
\framesubtitle{(Solutions available at \url{https://osf.io/fcrj6/})}
  \begin{block}{Exercise 3.1}
    ...
    \item Compute and plot the conditional and predictive power for replication success 
      using the controlled ($\alpha$ = 0.065) threshold. Use the function
