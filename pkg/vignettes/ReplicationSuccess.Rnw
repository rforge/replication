...
replication studies \citep{Micheloud2020} were added subsequently. Recently, substantial 
changes to many of the existing functions were made due to a recalibration of
the sceptical $p$-value approach \citep{Held_etal2020}.
Use \texttt{news(package = "ReplicationSuccess")} to see a history of the 
changes. \texttt{ReplicationSuccess} has not yet been released on CRAN but
can be installed from R-Forge by running the command
...
This may require some adaptation for certain effect sizes.
...
distributed. For correlation coefficients this is the case after applying the
Fisher $z$-transformation, which leads to their variance asymptotically being only a function 
...
original and replication study, achieve statistical significance (in the same direction).
...
\paragraph{Analysis}
Another analysis approach is to examine the statistical compatibility
of original and replication effect estimate. A popular way of doing this
is to check whether the replication
estimate is contained within its prediction interval based on the original
estimate \citep{Patil2016, Pawel2020}. With the function \texttt{predictionInterval}, a prediction
...
For the four data sets, we can easily compute 95\% prediction intervals and then check 
...
for (p in unique(RProjects$project)) {
    ...
    color <- ifelse(within == TRUE, "#00000099", "#DF536BCC")
...
also see \citet{Held2020, Held_etal2020} for technical details.
...
\begin{itemize}
  ...
  one (\ie if  there is no shrinkage).
...
\citet{Held_etal2020} further studied properties of the different
...
smaller than the value of the golden level
...
@ 
For many studies, replication success is declared both based on
significance and on the sceptical $p$-value. However, six replications
show discrepant results. The sceptical $p$-value may not indicate
replication success when there is substantial shrinkage of the
replication effect estimate relative to the original one, even if both
estimates are significant (this is the case for one study in the
psychology project, two studies in the social sciences project, one
study in the philosophy project).  On the other hand, provided there is not
much shrinkage, it may indicate replication success for
non-significant original or replication studies (this is the case for two
studies in the psychology project).
...
$d = \hat{\theta}_r/\hat{\theta}_o$ \citep{Held_etal2020}.
% (the 
...
the replication is deemed successful.
...
study should be stopped prematurely for futility \citep{Micheloud2020}.
The function 
...
replication study. The function \texttt{sampleSizeSignificanceInterim} can 
...
It is often more realistic to assume that the effects underlying the estimates 
from original and replication studies are not exactly the same but that there is 
between-study heterogeneity. This can be caused, for example, if the replication
...
\texttt{h}, the relative between-study heterogeneity variance $h = \tau^2/\sigma^2$, 
\ie the ratio of the heterogeneity variance to the variance of the original effect estimate.
By default, \texttt{h} is set to zero, however, if between-study heterogeneity is
expected, \eg a different population of study participants is used, this should
be considered in the design. See also \citet{Pawel2020} for more details.

\subsection{Data-driven shrinkage with empirical Bayes}
The functions \texttt{sampleSizeSignificance} and 
...
\section{Outlook}
% This vignette is meant to give an overview of the currently implemented
% functionality of the \texttt{ReplicationSuccess} package. The
% theory underlying the reverse-Bayes approach to quantify replication success
% was only explained on a high-level and we refer to the articles
% \citet{Held2020, Held_etal2020} for details. 
Development on \texttt{ReplicationSuccess} will continue and we plan to 
submit a stable version to CRAN at some point in the future. We invite anyone with
