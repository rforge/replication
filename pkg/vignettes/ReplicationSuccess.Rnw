...
all functions of the package use them as input.
<< "data-loading" >>=
...
@
\todo{unify suffix notation for original/replication}
...
for (p in unique(ReplicationProjects$project)) {
  ...
  plot(r_R ~ r_O, data = data_project, ylim = c(-0.5, 1), col = significant,
       xlim = c(-0.1, 1), main = p, xlab = expression(italic(r)[o]), cex=0.7, pch=19, 
...
<<>>=
sqrt.cH <- qnorm(1-2*(0.025^2))
(threshS <- t2p(sqrt.cH/2, alternative="one.sided"))
@ 
...
identifying $p_S = \Sexpr{signif(threshS,3)}$ (one.sided) as threshold for one-sided type I error control at $(1/40)^2=1/1600$
...
abline(h = threshS, lty = 3)
stripchart(ps ~ project, data = ReplicationProjects, vertical = TRUE, add = TRUE,
           pch = 19, method = "jitter", jitter = 0.2, cex=.6)

for (p in unique(ReplicationProjects$project)) {
  data_project <- subset(ReplicationProjects, project == p)
  cat(paste0(p, ": \n"))
  sel1 <- (data_project$ps < threshS)
  cat(paste0(round(mean(sel1)*100, 2), 
             "% smaller than 0.0653 (one-sided sceptical p-value) \n"))
  sel2 <- (data_project$pval_O/2 < 0.025)&(data_project$pval_R/2 < 0.025)
  cat(paste0(round(mean(sel2)*100, 2), 
             "% smaller than 0.025 (both one-sided traditional p-values) \n \n"))
  if(sum(sel1!=sel2)>0)
      print(data_project[(sel1!=sel2),])
}
@ 
...
even if both studies are significant, but with substantial shrinkage of the replication 
effect estimate relative to the original one. 
...
<< >>=
sampleSizeReplicationSuccess(to = 2.5, power = 0.8, level = threshS, alternative = "one.sided",
...
sceptical $p$-value smaller or equal 0.0653 as a function of the $p$-value or test statistic
of original study.
\begin{figure}[!h]
<< "plot-powerReplicationSuccess", echo = FALSE, fig.height = 4 >>=
## plot power
plot(po, powerReplicationSuccess(po = po/2, designPrior = "conditional",
                                 level = threshS, alternative = "one.sided")*100,
     type = "l", ylim = c(0, 100), lwd = 1.5, ylab = "Power (%)", las = 1,
     xlab = expression(italic(p)[o]))
axis(side = 3, at = seq(0.0, 0.05, by = 0.01), 
     labels = c("", round(p2t(p = seq(0.01, 0.05, by = 0.01)), 2)))
mtext(text = expression(paste("|", italic(t)[o], "|")), side = 3, line = 2)
lines(po, powerReplicationSuccess(po = po/2, designPrior = "predictive",
                                  level = threshS, alternative = "one.sided")*100, 
      lwd = 2, lty = 2)
...
(at the one-sided 0.0653 level) as a function of the two-sided $p$-value or test statistic of original study.}
...
## plot sample size 
plot(po, sampleSizeReplicationSuccess(po = po/2, designPrior = "conditional",
                                      level = threshS, alternative = "one.sided", 
                                      power = 0.8),
...
axis(side = 3, at = seq(0.0, 0.05, by = 0.01), 
     labels = c("", round(p2t(p = seq(0.01, 0.05, by = 0.01)), 2)))
mtext(text = expression(paste("|", italic(t)[o], "|")), side = 3, line = 2)
lines(po, sampleSizeReplicationSuccess(po = po/2, designPrior = "predictive", 
                                       level = threshS, alternative = "one.sided", 
                                       power = 0.8), 
      lwd = 2, lty = 2)
