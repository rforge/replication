...
\begin{minipage}{0.65\textwidth}
  ...
  signficance and confidence intervals, as well as more recently developed methods 
  such as the sceptical $p$-value \citep{Held2019} are included.
...
However, there is no consensus which statistical analysis approach
...
In this vignette the usage of the package is illustrated on several data sets
...
effect sizes. Specifically, after a suitable transformation the effect estimates are 
...
This framework is also common to many meta-analysis methods and
can for example be applied to mean differences, hazard ratios
(log transformation), odds ratios (log transformation), or correlation coefficients
(Fisher $z$-transformation). 
WRITE MORE DETAILS.

Moreover, most functions in \texttt{ReplicationSuccess} take unitless quantities 
as inputs. In particular, the test statistics 
$t_o = \hat{\theta}_o/\sigma_o$, $t_r = \hat{\theta}_r/\sigma_r$, and the variance
ratio $c = \sigma^2_o/\sigma^2_r$ ($\hat{\theta}$ denotes an effect estimate
...
As in the experimental economics replication project, a prediction market to estimate peer beliefs about the replicability of the original studies was conducted and the resulting belief estimates are also provided in the package.
...
The data can be loaded with the command \texttt{data("ReplicationProjects")}. For a
description of the variables see the documentation with \texttt{?ReplicationProjects}.
It is a good idea to first compute the unitless quantities $t_o$, $t_r$ and $c$, since 
all functions of the package usem them as input.
<< "data-loading" >>=
library(ReplicationSuccess)
data("ReplicationProjects")
str(ReplicationProjects)
...
@
Note that each variable ending with an \texttt{\_O} is associated with the original,
while each variable ending with an \texttt{\_R} is associated with the replication.
...
for (p in unique(ReplicationProjects$project)) {
  ...
  significant <- factor(data_project$pval_R < 0.05)
  plot(r_R ~ r_O, data = data_project, ylim = c(-0.5, 1), col = significant,
       xlim = c(-0.1, 1), main = p, xlab = expression(italic(r)[o]),
       ylab = expression(italic(r)[r]))
  legend("topleft", legend = "significant", pch = 20, col = 2, bty = "n")
  abline(h = 0, lty = 2)
...
do not achieve statistical significance at the 5\% level, while almost all original
estimates did. 
...
can be analysed, we will first discuss the particular analysis approach. 
...
the same statistical signficance status as the original estimate and also goes
...
For the four data sets, we can simply check whether the $p$-values of original and
...
for (p in unique(ReplicationProjects$project)) {
  data_project <- subset(ReplicationProjects, project == p)
    significant_O <- data_project$pval_O < 0.05
  significant_R <- data_project$pval_R < 0.05
  success <- (significant_O == significant_R) & 
             (sign(data_project$z_O) == sign(data_project$z_R))
  cat(paste0(p, ": \n"))
  cat(paste0(round(mean(significant_O)*100, 2), "% orginal studies significant (", 
...
was a false positive (\eg with 95\% probability if the significance level is 5\%), 
...
by just increasing the sample size.
...
just mentioned approaches. The argument \texttt{designPrior} allowes to carry out 
...
shrinkage can be specified with the argument \texttt{shrinkage} for the predictive
and conditional design prior.
...
<< >>=
sampleSizeSignificance(to = 2.5, power = 0.8, level = 0.05, designPrior = "conditional")
sampleSizeSignificance(to = 2.5, power = 0.8, level = 0.05, designPrior = "predictive")
sampleSizeSignificance(to = 2.5, power = 0.8, level = 0.05, designPrior = "conditional",
                       shrinkage = 0.75)
sampleSizeSignificance(to = 2.5, power = 0.8, level = 0.05, designPrior = "EB")
@

Figure \ref{fig:powerSignificance} shows the power to achieve significance in the
replication as a function of either the (two-sided) $p$-value or the test statistic
...
# plot power
plot(po, powerSignificance(po = po, designPrior = "conditional")*100,
     type = "l", ylim = c(0, 100), lwd = 1.5, ylab = "Power (%)", 
     xlab = expression(italic(p)[o]), las = 1)
axis(side = 3, at = seq(0.0, 0.05, by = 0.01), 
     labels = c("", round(p2t(p = seq(0.01, 0.05, by = 0.01)), 2)))
mtext(text = expression(paste("|", italic(t)[o], "|")), side = 3, line = 2)
# abline(h = seq(0, 100, 25), col = "#333333B3", lty = 3)
lines(po, powerSignificance(po = po, designPrior = "predictive")*100, lwd = 2, lty = 2)
lines(po, powerSignificance(po = po, designPrior = "EB")*100, lwd = 1.5, lty = 4)
legend("topright", legend = c("conditional", "predictive", "EB"), 
       title = "Design prior", lty = c(1, 2, 4), lwd = 1.5, bty = "n")
@
\caption{Power to achieve significance at 5\% level in replication as a function of (two-sided) $p$-value or test statistic of original study using the same sample size
...
# plot sample size 
plot(po, sampleSizeSignificance(po = po, designPrior = "conditional", power = 0.8),
...
lines(po, sampleSizeSignificance(po = po, designPrior = "predictive", power = 0.8), 
      lwd = 2, lty = 2)
lines(po, sampleSizeSignificance(po = po, designPrior = "EB", power = 0.8),
      lwd = 1.5, lty = 4)
legend("topleft", legend = c("conditional", "predictive", "EB"), 
       title = "Design prior", lty = c(1, 2, 4), lwd = 1.5, bty = "n")
@
\caption{Relative sample size to achieve significance at 5\% level with 80\% power as
a function of (two-sided) $p$-value or test statistic of original study.}
...
interval of the replication test statistic can be computed under different predictive
...
for (p in unique(ReplicationProjects$project)) {
  ...
  PI <- predictionInterval(to = data_project$to, c = data_project$c)*
        data_project$z_se_R # multiplying by standard error to transform to z-scale
...
This in turn can lead to the very different effect estimates being compatible, 
...
needs to be changed compared to the original study.
<< >>=
sampleSizePI(to = 2.5, conf.level = 0.95, designPrior = "predictive")
sampleSizePI(to = 2.1, conf.level = 0.95, designPrior = "predictive")
@
...
\begin{figure}[!h]
<< "plot-sampleSizePI", echo = FALSE, fig.height = 4 >>=
# required relative sample size for 0.95 PI not to include 0 as function of 
# test statistic
to <- seq(0, 5, 0.01)
plot(to, sampleSizePI(to = to), type = "l", log = "y", lwd = 1.5,
     ylab = expression(paste("Relative sample size ", n[r]/n[o])),
     xlab = expression(italic(t)[o]), ylim = c(0.15, 1000), yaxt = "n")
axis(side = 2, las = 1, at = c(0.1, 1, 10, 100, 1000), 
     labels = c("1/10", "1", "10", "100", "1000"))
abline(h = 1, lty = 3)
# lines(to, sampleSizePI(to = to, designPrior = "EB"), lty = 2, lwd = 1.5)
...
EXPAND THIS SECTION
\paragraph{Analysis}
The \emph{sceptical $p$-value}, a new quantitative measure of replication success was recently proposed by \citet{Held2019}. The \emph{sceptical $p$-value} arises from 
...
This prior corresponds to the objection of a scpetic who argues
...
# Functions
ssv <- function(t_o, s_o, a) {
  tau2 <- s_o^2/(t_o^2/qnorm(p = a/2, lower.tail = FALSE)^2 - 1)
  return(tau2)
}
...
tau <- sqrt(ssv(t_o, s_o, alpha))
s2_p <- 1/(1/s_o^2 + 1/tau^2)
mu_p <- s2_p*theta_o/s_o^2
ps <- signif(pSceptical(to = t_o, tr = t_r, c = c), 2)
ps_tilde <- signif(pSceptical(to = t_o, tr = t_r, c = c, 
                              alternative = "one.sided"), 2)


# Plot
df <- data.frame(estimate = factor(c("Original Study", "Posterior", "Sceptical Prior", 
                                     "Replication Study"),
                                   levels = c("Original Study", "Posterior", 
                                              "Sceptical Prior", "Replication Study")),
                 theta = c(theta_o, mu_p, 0, theta_r),
                 lower = c(theta_o - za*s_o, mu_p - za*sqrt(s2_p), 0 - za*tau, theta_r - za*s_r),
                 upper = c(theta_o + za*s_o, mu_p + za*sqrt(s2_p), 0 + za*tau,
                           theta_r + za*s_r),
                 p = c(signif(2*pnorm(q = t_o, lower.tail = FALSE), 1), 
                       NA, NA, signif(2*pnorm(q = t_r, lower.tail = FALSE), 2)))
...
text(x = 4 + 0.3, y = 0.9, labels = bquote(italic(p)[S] == .(ps)), col = 2)
...
@
\caption{Example of assessment of replication success with sceptical $p$-value.}
\end{figure}
...
\citet{Held2019b} further expanded on the calibration of the sceptical $p$-value,
identifying $p_S = 0.065$ (one.sided) as threshold for type I error control at 5\% 
if replication and original estimate have equal variances. 

The sceptical $p$-value can be easily computed with the function \texttt{pSceptical}.
For the analysis of replication studies, it usually only makes sense to compute the 
one-sided version.
<< "plot-pSceptical-projects", fig.height = 4 >>=
# computing one.sided sceptical p-value for replication projects
ReplicationProjects$ps <- with(ReplicationProjects, 
                               pSceptical(to = to, tr = tr, c = c, alternative = "one.sided"))
boxplot(ps ~ project, data = ReplicationProjects, las = 1, cex.axis = 0.7, ylim = c(0, 1),
        xlab = "Project", ylab = expression(italic(p)[S]), outline = FALSE)
abline(h = 0.065, lty = 3)
stripchart(ps ~ project, data = ReplicationProjects, vertical = TRUE, add = TRUE,
           pch = 1, method = "jitter", jitter = 0.2)

for (p in unique(ReplicationProjects$project)) {
  data_project <- subset(ReplicationProjects, project == p)
  cat(paste0(p, ": \n"))
  cat(paste0(round(mean(data_project$ps < 0.065)*100, 2), 
             "% smaller than 0.065 (one-sided sceptical p-value) \n"))
  cat(paste0(round(mean(data_project$pval_R/2 < 0.05)*100, 2), 
             "% smaller than 0.05 (one-sided traditional p-value) \n \n"))
}
@
Thresholding the one-sided traditional $p$-values and the one-sided sceptical $p$-values
at the appropriate thresholds for type I error control,
we can see that the latter is more stringent than the former.
...
<< >>=
sampleSizeReplicationSuccess(to = 2.5, power = 0.8, level = 0.065, alternative = "one.sided",
...
sceptical $p$-value smaller or equal 0.065 as a function of the $p$-value or test statistic
of original study.
\begin{figure}[!h]
<< "plot-powerReplicationSuccess", echo = FALSE, fig.height = 4 >>=
# plot power
plot(po, powerReplicationSuccess(po = po, designPrior = "conditional",
                                 level = 0.065, alternative = "one.sided")*100,
     type = "l", ylim = c(0, 100), lwd = 1.5, ylab = "Power (%)", las = 1,
     xlab = expression(italic(p)[o]))
axis(side = 3, at = seq(0.0, 0.05, by = 0.01), 
     labels = c("", round(p2t(p = seq(0.01, 0.05, by = 0.01)), 2)))
mtext(text = expression(paste("|", italic(t)[o], "|")), side = 3, line = 2)
lines(po, powerReplicationSuccess(po = po, designPrior = "predictive",
                                  level = 0.065, alternative = "one.sided")*100, 
      lwd = 2, lty = 2)
...
(at the one-sided 0.065 level) as a function of (two-sided) $p$-value or test statistic of original study.}
\label{fig:powerReplicationSuccess}
\end{figure}

Figure \ref{fig:sampleSizeReplicationSuccess} shows the required sample size to achieve
a one-sided sceptical $p$-value with 80\% power. 
\begin{figure}[!h]
<< "plot-sampleSizeReplicationSuccess", echo = FALSE, fig.height = 4 >>=
po <- seq(0.0001, 0.05, 0.0001)

# plot sample size 
plot(po, sampleSizeReplicationSuccess(po = po, designPrior = "conditional",
...
axis(side = 3, at = seq(0.0, 0.05, by = 0.01), 
     labels = c("", round(p2t(p = seq(0.01, 0.05, by = 0.01)), 2)))
mtext(text = expression(paste("|", italic(t)[o], "|")), side = 3, line = 2)
lines(po, sampleSizeReplicationSuccess(po = po, designPrior = "predictive", 
                                       level = 0.065, alternative = "one.sided", 
                                       power = 0.8), 
      lwd = 2, lty = 2)
legend("topright", legend = c("conditional", "predictive"), 
...
(at the one-sided 0.065 level) with 80\% power as
a function of (two-sided) $p$-value or test statistic of original study.}
