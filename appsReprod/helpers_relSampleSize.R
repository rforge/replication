...
pSignificant <- function(p=NULL, t=p.to.z(p), c=1, sig.level=0.05){
  pSig <- pnorm(p.to.z(sig.level), mean=sqrt(c)*t, sd=sqrt(1+c), lower.tail=FALSE)
  return(pSig)
}
...
sampleSizePlot <- function(p.min=0.00001, p.max=0.05, c.max){
  ...
  par(las=1)
  # par(las=1, pty="s")
  plot(0,0, col="white", xlim=c(0, p.max), ylim=c(0, c.max), ylab="Relative sample size", xlab="two-sided p-value in original study", axes=FALSE, cex.lab=1.2)
  if(p.max <= 0.03)
  ...
  if(c.max < 2)
    axis(2, at=seq(from=0, to=2, by=0.2), labels=as.character(seq(from=0, to=2, by=0.2)), cex.axis=1.2)
  if(c.max >= 2 && c.max < 6)
    axis(2, at=seq(from=0, to=6, by=0.5), labels=as.character(seq(from=0, to=6, by=0.5)), cex.axis=1.2)
  if(c.max >= 6 && c.max < 16)
     axis(2, at=seq(from=0, to=c.max+1, by=1), labels=as.character(seq(from=0, to=c.max+1, by=1)), cex.axis=1.2)
  if(c.max >= 16 && c.max < 30)
    axis(2, at=seq(from=0, to=c.max+1, by=2), labels=as.character(seq(from=0, to=c.max+1, by=2)), cex.axis=1.2)
  if(c.max >= 30 && c.max < 60)
    axis(2, at=seq(from=0, to=200, by=5), labels=as.character(seq(from=0, to=200, by=5)), cex.axis=1.2)
  if(c.max >= 60)
    axis(2, at=seq(from=0, to=200, by=10), labels=as.character(seq(from=0, to=200, by=10)), cex.axis=1.2)
  box()
}

# ClassicalTarget <- function(n, n0, delta, sd, sig.level=0.05, power, type="two.sample"){
#   term <- ClassicalPower(n0=n0, n=n, delta=delta, sd=sd,
#                          sig.level=sig.level, type=type)
#   return(term - power)
# }
...
segm.rss <- function(p, alpha=0.05, power=0.8, prior="point", col=2){
  c <- relSampleSize(p=p, alpha=alpha, power=power, prior=prior)
